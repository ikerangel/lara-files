...
class InitialFileSystemScan extends Command
{
    ...
    private function displayResults(array $stats, float $duration, int $eventsCreated): void
    {
        ...
        $this->table(
            ...
            [
                ...
                ['Items per second', $stats['directories'] + $stats['files'] > 0 ? round(($stats['directories'] + $stats['files']) / $duration) : 0],
            ]
        );
    }
    ...
    private function showSampleEvents(): void
    {
        ...
        $sampleEvents = DB::table('stored_events')
            ...
            ->orderBy('id', 'desc')
            ->limit(5)
            ->get(['event_class', 'file_path', 'file_type', 'created_at']);
        ...
        $tableData = $sampleEvents->map(function ($event) {
            return [
                ...
                'Path' => $event->file_path ? basename($event->file_path) : 'N/A',
                'Type' => $event->file_type ?? 'N/A',
